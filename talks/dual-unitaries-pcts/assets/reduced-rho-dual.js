...
for (let y = 0; y < numDeep; y++) {
    ...
    for (let x = 0; x < numWide; x++) {
        ...
        if ((endRho - x < Math.ceil(y / 2)) ||  (x - startRho < Math.floor(y / 2))) {
        ...
        // light cone logic
        const [bottomLeftContraction, bottomRightContraction] = bottomContraction.children()
        if ((y!==0) || ((x >= startRho) && (x <= endRho))) {
            topContraction.attr({visibility: 'hidden'})
        }
        if (x - startRho === Math.floor(y / 2) && (y <= endRho - startRho) && y !== numDeep - 1) {
            bottomLeftContraction.attr({visibility: 'visible'})
        }
        if ((endRho - x === Math.ceil(y / 2)) && (y <= endRho - startRho) && y !== numDeep - 1) {
            bottomRightContraction.attr({visibility: 'visible'})
        }
