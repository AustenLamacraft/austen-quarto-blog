...
P5 = require("p5")
function* createSketch(sketch, divId, slideId) {
...
```{ojs}
import { expectation } from './js/expectation.js'
html`<div id="svg-container"></div>`
d3.xml('./images/expectation.svg')
  .then(data => {
    d3.select("#svg-container").node().append(data.documentElement)
  });
// graphic = {
//   const test = await FileAttachment("text.txt").text();
//   console.log(test)
//   const data = (new DOMParser).parseFromString(test, "image/svg+xml");
//   d3.select("#svg-container").node().append(data.documentElement)
// }
  circuit = SVG('#gate')
    ...
    // Hide original 
    gateAndContractions = circuit.findOne('#gate-and-contractions').hide()
    

    // The four contractions
    topLeft = circuit.findOne('#top-left')
    topRight = circuit.findOne('#top-right')
    bottomLeft = circuit.findOne('#bottom-left')
    bottomRight = circuit.findOne('#bottom-right')
...
createSketch(elementary, 'elementary-ca-demo', 'elementary-ca');
```
