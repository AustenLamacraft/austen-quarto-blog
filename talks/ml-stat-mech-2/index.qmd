...
subtitle: "TCM graduate lectures"
...
- [Lecture 1](../ml-stat-mech-1/) introduced idea of Variational Inference (VI)
...
![Autoencoder schema (figure from [Wikipedia](https://en.wikipedia.org/wiki/Autoencoder))](assets/Autoencoder_schema.png){width=30%}
...
- Some issues:
    ...
    1. Loss function doesn't require that the latent space is used *at all*. If decoder model $p_\theta(\bx|\bz)$ is rich enough may have $p_\theta(\bx|\bz)\approx p_\text{D}(\bx)$. By Bayes' theorem posterior is
    $$
...
- In [Lecture 1](../ml-stat-mech-1/) I suggested that good probabilistic models could give better compression
...
$$
$W_t$ is $\R^H$ dimensional Brownian motion, $\mu_\theta(z_t)$ is a parameterized drift
